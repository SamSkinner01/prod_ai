...
class GPUAugmentation(nn.Module):
    """
    GPU-based data augmentation using torchvision v2 transforms.
    ...
    def __init__(self, cfg: DictConfig, training: bool = True):
        ...
        if training:
            ...
            # Random affine
            if cfg.random_affine.enabled:
                transforms.append(v2.RandomAffine(
                    degrees=cfg.random_affine.degrees,
                    translate=tuple(cfg.random_affine.translate),
                    scale=tuple(cfg.random_affine.scale),
                ))
...
def main(cfg: DictConfig) -> None:
    ...
    model = MNISTNet(dropout_p=cfg.model.dropout_p).to(device)
    log.info(f"Model parameters: {sum(p.numel() for p in model.parameters()):,}")

    # Create GPU augmentation modules
    train_augmentation = GPUAugmentation(cfg.augmentation, training=True).to(device)
    val_augmentation = GPUAugmentation(cfg.augmentation, training=False).to(device)
    ...
    for epoch in range(1, cfg.training.epochs + 1):
        ...
        val_loss, val_acc, samples = validate(
            ...
            collect_samples=(epoch % 5 == 0 or epoch == 1)  # Visualize every 5 epochs
        )
